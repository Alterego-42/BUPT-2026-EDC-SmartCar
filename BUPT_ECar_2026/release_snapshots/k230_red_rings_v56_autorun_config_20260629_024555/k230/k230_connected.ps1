...
$RemoteMain = "/sdcard/k230_target_uart.py"
$RemoteConfig = "/sdcard/k230_target_config.py"
...
function New-DebugScript {
    ...
    $text = $text -replace '(?m)^CONFIG_OVERRIDES_ENABLED\s*=\s*True\s*$', 'CONFIG_OVERRIDES_ENABLED = False'
    if ($EnableDebug -and -not $ForceCommand) {
        $text = $text -replace '(?m)^USB_DEBUG_ENABLED\s*=\s*False\s*$', 'USB_DEBUG_ENABLED = True'
        $text = $text -replace '(?m)^TARGET_CANDIDATE_DEBUG\s*=\s*False\s*$', 'TARGET_CANDIDATE_DEBUG = True'
    }
    $text = $text -replace '(?m)^DEFAULT_RUN_SECONDS\s*=\s*None\s*$', "DEFAULT_RUN_SECONDS = $RunSeconds"
    if ($DisableControl) {
        $text = $text -replace '(?m)^CONTROL_OUTPUT_ENABLED\s*=\s*True\s*$', 'CONTROL_OUTPUT_ENABLED = False'
    }
    if ($ForceCommand) {
        $text = $text -replace '(?m)^FORCE_COMMAND_ENABLED\s*=\s*False\s*$', 'FORCE_COMMAND_ENABLED = True'
        $text = $text -replace '(?m)^FORCE_YAW\s*=\s*[-+0-9.]+\s*$', ("FORCE_YAW = {0:F2}" -f $YawCommand)
        $text = $text -replace '(?m)^FORCE_PITCH\s*=\s*[-+0-9.]+\s*$', ("FORCE_PITCH = {0:F2}" -f $PitchCommand)
    }
...
function New-DiagnoseScript {
    $text = Get-Content -Raw $MainScript
    $text = $text -replace '(?m)^CONFIG_OVERRIDES_ENABLED\s*=\s*True\s*$', 'CONFIG_OVERRIDES_ENABLED = False'
    $text = $text -replace '(?m)^DIAGNOSE_ONCE\s*=\s*False\s*$', 'DIAGNOSE_ONCE = True'
    $text = $text -replace '(?m)^CONTROL_OUTPUT_ENABLED\s*=\s*True\s*$', 'CONTROL_OUTPUT_ENABLED = False'
    $text = $text -replace '(?m)^USB_DEBUG_ENABLED\s*=\s*False\s*$', 'USB_DEBUG_ENABLED = False'
