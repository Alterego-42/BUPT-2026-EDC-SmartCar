...
# The red marks are only used to confirm/score the black panel candidate.  The
# aiming target is always the black panel/frame center.
...
CONFIG_PATH = "/sdcard/k230_target_config_v72.py"
...
def cleanup_camera(sensor=None):
    try:
        if sensor:
            sensor.stop()
    except Exception:
...
def point_in_target_bounds(cx, cy, target, margin=0.0):
    if not target:
        return False
    x0, y0, x1, y1 = target_bounds(target)
    return (
...
def fixed_laser_ref(target=None):
    cx = FIXED_LASER_CX
    cy = FIXED_LASER_CY
    ref_size = 0.0
    ratio = 0.0

    if FIXED_LASER_RANGE_REF_ENABLED and target:
        t_src, _t_cx, _t_cy, t_w, t_h, _corners, _score = target
        if FIXED_LASER_RANGE_BY_FRAME_IMAGE_W:
            ref_size = float(t_w)
            far_size = FIXED_LASER_FAR_TARGET_W
            near_size = FIXED_LASER_NEAR_TARGET_W
        else:
            ref_size = float(t_h)
            if t_src == "red_rings":
                ref_size *= FIXED_LASER_RED_RING_PANEL_H_SCALE
            far_size = FIXED_LASER_FAR_TARGET_H
            near_size = FIXED_LASER_NEAR_TARGET_H
        span = near_size - far_size
        if span != 0.0:
            ratio = clamp((ref_size - far_size) / span, 0.0, 1.0)
            cx = FIXED_LASER_FAR_CX + (FIXED_LASER_NEAR_CX - FIXED_LASER_FAR_CX) * ratio
            cy = FIXED_LASER_FAR_CY + (FIXED_LASER_NEAR_CY - FIXED_LASER_FAR_CY) * ratio

    return ("fixed", cx, cy, 4, 4, int(ref_size), int(ratio * 1000.0))
...
def target_hold_allowed(target):
    ...
    return (
        target[0] == "red_rings"
        or target[0] == "red_panel_center"
        or target[0] == "red_panel_frame_center"
        or target[0] == "frame_center"
    )


def red_rings_part_track_allowed(target):
    if not target or target[0] != "red_rings_part":
        return False
    _src, _cx, _cy, w, h, _corners, score = target
    return (
        w >= RED_RINGS_PART_TRACK_MIN_W
        and h >= RED_RINGS_PART_TRACK_MIN_H
        and score >= RED_RINGS_PART_TRACK_MIN_SCORE
    )


def red_rings_part_fixed_ref_allowed(target, laser):
    if (
        not target
        ...
        return False
    _src, _cx, _cy, _w, h, _corners, score = target
    return h >= RED_RINGS_PART_FIXED_REF_MIN_H and score >= RED_RINGS_PART_FIXED_REF_MIN_SCORE


def red_rings_part_control_allowed(target, laser):
    if not RED_RINGS_PART_CONTROL_ENABLED or not target or target[0] != "red_rings_part":
        return False
    return (
        red_rings_part_edge_command(target, laser)
        or red_rings_part_track_allowed(target)
        or red_rings_part_fixed_ref_allowed(target, laser)
    )


def red_panel_frame_is_complete(panel):
    if not panel:
        return False

    _src, _cx, _cy, w, h, corners, _score = panel
    if not corners:
        return False

    x, y, bw, bh = rect_bounds_from_corners(corners)
    edge = RED_PANEL_FRAME_COMPLETE_EDGE_PX
    if x <= edge or y <= edge:
        return False
    if x + bw >= FRAME_W - edge or y + bh >= FRAME_H - edge:
        return False

    area = bw * bh
    if not valid_red_panel_frame_box(x, y, bw, bh, area):
        return False
...
def find_anchor_panel_from_red_target(img, red_target, paper):
    debug = DIAGNOSE_ONCE or TARGET_CANDIDATE_DEBUG

    if not RED_PANEL_ANCHOR_ENABLED or not red_target:
        return None

    _src, red_cx, red_cy, red_w, red_h, red_corners, red_score = red_target
    if red_corners:
        rx0, ry0, rw, rh = rect_bounds_from_corners(red_corners)
        rx1 = rx0 + rw
        ry1 = ry0 + rh
    else:
        rx0 = red_cx - red_w / 2.0
        ry0 = red_cy - red_h / 2.0
        rx1 = red_cx + red_w / 2.0
        ry1 = red_cy + red_h / 2.0

    x0 = int(clamp(rx0 - RED_PANEL_ANCHOR_PAD_X, 0.0, float(FRAME_W - 1)))
    y0 = int(clamp(ry0 - RED_PANEL_ANCHOR_PAD_Y, 0.0, float(FRAME_H - 1)))
    x1 = int(clamp(rx1 + RED_PANEL_ANCHOR_PAD_X, float(x0 + 1), float(FRAME_W)))
    y1 = int(clamp(ry1 + RED_PANEL_ANCHOR_PAD_Y, float(y0 + 1), float(FRAME_H)))
    if x1 <= x0 or y1 <= y0:
        if debug:
            print("$ANCHORDBG,reason=roi_empty")
        return None

    if PAPER_ROI_ENABLED and paper:
        px0, py0, px1, py1 = paper_bounds(paper)
        x0 = int(max(float(x0), px0 - RED_PANEL_FRAME_PAPER_RELAX_PX))
        y0 = int(max(float(y0), py0 - RED_PANEL_FRAME_PAPER_RELAX_PX))
        x1 = int(min(float(x1), px1 + RED_PANEL_FRAME_PAPER_RELAX_PX))
        y1 = int(min(float(y1), py1 + RED_PANEL_FRAME_PAPER_RELAX_PX))
        if x1 <= x0 or y1 <= y0:
            if debug:
                print("$ANCHORDBG,reason=paper_roi_empty")
            return None

    roi = (x0, y0, x1 - x0, y1 - y0)
    try:
        blobs = img.find_blobs(
            [SQUARE_THRESHOLD, RED_PANEL_ANCHOR_THRESHOLD],
            roi=roi,
            pixels_threshold=SQUARE_PART_MIN_PIXELS,
            area_threshold=SQUARE_PART_MIN_AREA,
            merge=False,
        ...
        if debug:
            print("$ANCHORDBG,reason=find_blobs_error")
        return None
    ...
    for b in blobs:
        ...
        if density < SQUARE_MIN_DENSITY or density > 98.0:
            continue
        parts.append((bx, by, bw, bh, pixels, area))

    if len(parts) < 1:
        if debug:
            print("$ANCHORDBG,reason=no_parts,roi=%d:%d:%d:%d,blobs=%d" %
                  (x0, y0, x1 - x0, y1 - y0, len(blobs)))
        return None
    ...
    dark_count = sum([p[4] for p in parts])
    bw = float(max_x - min_x + 1)
    bh = float(max_y - min_y + 1)
    area = bw * bh
    if bw < RED_PANEL_ANCHOR_MIN_W or bh < RED_PANEL_ANCHOR_MIN_H:
        if debug:
            print("$ANCHORDBG,reason=size,x=%d,y=%d,w=%d,h=%d,parts=%d,dark=%d" %
                  (int(min_x), int(min_y), int(bw), int(bh), len(parts), int(dark_count)))
        return None
    if bw > RED_PANEL_FRAME_MAX_W or bh > RED_PANEL_FRAME_MAX_H:
        if debug:
            print("$ANCHORDBG,reason=max_size,x=%d,y=%d,w=%d,h=%d,parts=%d,dark=%d" %
                  (int(min_x), int(min_y), int(bw), int(bh), len(parts), int(dark_count)))
        return None
    if area < RED_PANEL_FRAME_MIN_AREA or area > RED_PANEL_FRAME_MAX_AREA:
        if debug:
            print("$ANCHORDBG,reason=area,x=%d,y=%d,w=%d,h=%d,area=%d,parts=%d,dark=%d" %
                  (int(min_x), int(min_y), int(bw), int(bh), int(area), len(parts), int(dark_count)))
        return None
    aspect = bw / bh if bh else 0.0
    if aspect < RED_PANEL_FRAME_MIN_ASPECT or aspect > RED_PANEL_ANCHOR_MAX_ASPECT:
        if debug:
            print("$ANCHORDBG,reason=aspect,x=%d,y=%d,w=%d,h=%d,aspect=%.2f,parts=%d,dark=%d" %
                  (int(min_x), int(min_y), int(bw), int(bh), aspect, len(parts), int(dark_count)))
        return None
    if min_x < 0 or min_y < 0 or max_x > FRAME_W or max_y > FRAME_H:
        if debug:
            print("$ANCHORDBG,reason=frame_bounds")
        return None
    if dark_count * 100.0 / area < RED_PANEL_ANCHOR_MIN_DARK_RATIO:
        if debug:
            print("$ANCHORDBG,reason=density,x=%d,y=%d,w=%d,h=%d,den=%.2f,parts=%d,dark=%d" %
                  (int(min_x), int(min_y), int(bw), int(bh), dark_count * 100.0 / area, len(parts), int(dark_count)))
        return None
    pcx = float(min_x) + bw / 2.0
    pcy = float(min_y) + bh / 2.0
    max_center_offset = max(8.0, min(bw, bh) * RED_PANEL_ANCHOR_MAX_RED_CENTER_RATIO)
    dx = red_cx - pcx
    dy = red_cy - pcy
    if (dx * dx + dy * dy) > (max_center_offset * max_center_offset):
        if debug:
            print("$ANCHORDBG,reason=center,x=%d,y=%d,w=%d,h=%d,pcx=%.1f,pcy=%.1f,rcx=%.1f,rcy=%.1f,max=%.1f" %
                  (int(min_x), int(min_y), int(bw), int(bh), pcx, pcy, red_cx, red_cy, max_center_offset))
        return None
    ...
    score = int(red_score + dark_count * 8 + area * 0.4)
    if debug:
        print("$ANCHORDBG,ok=1,x=%d,y=%d,w=%d,h=%d,parts=%d,dark=%d,score=%d" %
              (int(min_x), int(min_y), int(bw), int(bh), len(parts), int(dark_count), score))
    return ("red_panel_anchor", pcx, pcy, bw, bh, corners, score)


def find_red_panel_edges_from_red_target(img, red_target, paper):
    debug = RED_PANEL_EDGE_DEBUG or DIAGNOSE_ONCE or TARGET_CANDIDATE_DEBUG

    if not RED_PANEL_EDGE_ESTIMATE_ENABLED or not red_target:
        return None
    ...
    x0 = int(clamp(rx0 - RED_PANEL_EDGE_PAD_X, 0.0, float(FRAME_W - 1)))
    y0 = int(clamp(ry0 - RED_PANEL_EDGE_PAD_Y, 0.0, float(FRAME_H - 1)))
    x1 = int(clamp(rx1 + RED_PANEL_EDGE_PAD_X, float(x0 + 1), float(FRAME_W)))
    y1 = int(clamp(ry1 + RED_PANEL_EDGE_PAD_Y, float(y0 + 1), float(FRAME_H)))
    if PAPER_ROI_ENABLED and paper:
        px0, py0, px1, py1 = paper_bounds(paper)
        x0 = int(max(float(x0), px0 - RED_PANEL_FRAME_PAPER_RELAX_PX))
        y0 = int(max(float(y0), py0 - RED_PANEL_FRAME_PAPER_RELAX_PX))
    ...
    if x1 <= x0 or y1 <= y0:
        if debug:
            print("$EDGEDBG,reason=roi_empty")
        return None
    ...
    try:
        blobs = img.find_blobs(
            [SQUARE_THRESHOLD, RED_PANEL_ANCHOR_THRESHOLD],
            roi=(x0, y0, x1 - x0, y1 - y0),
            pixels_threshold=RED_PANEL_EDGE_MIN_PIXELS,
            area_threshold=RED_PANEL_EDGE_MIN_AREA,
            merge=False,
            margin=0,
        )
    except Exception:
        if debug:
            print("$EDGEDBG,reason=find_blobs_error")
        return None
    ...
    bottom = None
    kept = 0
    min_overlap_y = max(4.0, red_h * RED_PANEL_EDGE_OVERLAP_RATIO)
    min_overlap_x = max(4.0, red_w * RED_PANEL_EDGE_OVERLAP_RATIO)
    ...
    for b in blobs:
        ...
        pixels = float(value(b, 4, bw * bh))
        area = bw * bh
        if pixels < RED_PANEL_EDGE_MIN_PIXELS:
            continue
        if area < RED_PANEL_EDGE_MIN_AREA or area > SQUARE_PART_MAX_AREA:
            continue
        density = pixels * 100.0 / area if area else 0.0
        if density < RED_PANEL_EDGE_MIN_DENSITY or density > RED_PANEL_EDGE_MAX_DENSITY:
            continue
        if not box_in_paper(bx, by, bw, bh, paper, -RED_PANEL_FRAME_PAPER_RELAX_PX):
            continue

        kept += 1
        bcx = bx + bw / 2.0
        bcy = by + bh / 2.0
        overlap_y = max(0.0, min(by + bh, ry1) - max(by, ry0))
        overlap_x = max(0.0, min(bx + bw, rx1) - max(bx, rx0))
        vertical_score = pixels * 4.0 + bh * 8.0 + overlap_y * 20.0 - abs(bcy - red_cy) * 0.5
        horizontal_score = pixels * 4.0 + bw * 8.0 + overlap_x * 20.0 - abs(bcx - red_cx) * 0.5

        if bcx < red_cx and overlap_y >= min_overlap_y and bh >= max(8.0, red_h * RED_PANEL_EDGE_MIN_SPAN_RATIO):
            if left is None or vertical_score > left[0]:
                left = (vertical_score, bx, by, bw, bh, pixels)
        if bcx > red_cx and overlap_y >= min_overlap_y and bh >= max(8.0, red_h * RED_PANEL_EDGE_MIN_SPAN_RATIO):
            if right is None or vertical_score > right[0]:
                right = (vertical_score, bx, by, bw, bh, pixels)
        if bcy < red_cy and overlap_x >= min_overlap_x and bw >= max(8.0, red_w * RED_PANEL_EDGE_MIN_SPAN_RATIO):
            if top is None or horizontal_score > top[0]:
                top = (horizontal_score, bx, by, bw, bh, pixels)
        if bcy > red_cy and overlap_x >= min_overlap_x and bw >= max(8.0, red_w * RED_PANEL_EDGE_MIN_SPAN_RATIO):
            if bottom is None or horizontal_score > bottom[0]:
                bottom = (horizontal_score, bx, by, bw, bh, pixels)

    if not left or not right:
        if debug:
            print("$EDGEDBG,reason=side_pair,left=%d,right=%d,top=%d,bottom=%d,kept=%d" %
                  (1 if left else 0, 1 if right else 0, 1 if top else 0, 1 if bottom else 0, kept))
        return None

    _ls, lx, ly, lw, lh, lpix = left
    _rs, rx, ry, rw, rh, rpix = right
    px0 = lx
    px1 = rx + rw
    pcx = (px0 + px1) / 2.0
    pw = px1 - px0
    if pw < RED_PANEL_EDGE_MIN_PAIR_W:
        if debug:
            print("$EDGEDBG,reason=pair_w,w=%d" % int(pw))
        return None

    if top and bottom:
        _ts, tx, ty, tw, th, tpix = top
        _bs, bx, by, bw, bh, bpix = bottom
        py0 = ty
        py1 = by + bh
        pixels = lpix + rpix + tpix + bpix
    elif top:
        _ts, tx, ty, tw, th, tpix = top
        py0 = ty
        ph_est = max(RED_PANEL_EDGE_MIN_PAIR_H, pw * RED_PANEL_EDGE_HEIGHT_FROM_WIDTH)
        py1 = py0 + ph_est
        pixels = lpix + rpix + tpix
    elif bottom:
        _bs, bx, by, bw, bh, bpix = bottom
        py1 = by + bh
        ph_est = max(RED_PANEL_EDGE_MIN_PAIR_H, pw * RED_PANEL_EDGE_HEIGHT_FROM_WIDTH)
        py0 = py1 - ph_est
        pixels = lpix + rpix + bpix
    else:
        side_y0 = min(ly, ry)
        side_y1 = max(ly + lh, ry + rh)
        side_h = side_y1 - side_y0
        ph_est = max(RED_PANEL_EDGE_MIN_PAIR_H, pw * RED_PANEL_EDGE_HEIGHT_FROM_WIDTH, side_h)
        py0 = red_cy - ph_est / 2.0
        py1 = red_cy + ph_est / 2.0
        pixels = lpix + rpix

    py0 = clamp(py0, 0.0, float(FRAME_H - 1))
    py1 = clamp(py1, py0 + 1.0, float(FRAME_H))
    ph = py1 - py0
    if ph < RED_PANEL_EDGE_MIN_PAIR_H:
        if debug:
            print("$EDGEDBG,reason=pair_h,h=%d" % int(ph))
        return None

    pcy = (py0 + py1) / 2.0
    max_center_offset = max(10.0, min(pw, ph) * RED_PANEL_EDGE_MAX_RED_CENTER_RATIO)
    dx = red_cx - pcx
    dy = red_cy - pcy
    if (dx * dx + dy * dy) > (max_center_offset * max_center_offset):
        if debug:
            print("$EDGEDBG,reason=center,pcx=%.1f,pcy=%.1f,rcx=%.1f,rcy=%.1f,max=%.1f" %
                  (pcx, pcy, red_cx, red_cy, max_center_offset))
        return None

    corners = [(px0, py0), (px1, py0), (px1, py1), (px0, py1)]
    score = int(red_score + pixels * 10 + pw * ph * 0.20)
    if debug:
        print("$EDGEDBG,ok=1,cx=%.1f,cy=%.1f,w=%d,h=%d,score=%d,kept=%d" %
              (pcx, pcy, int(pw), int(ph), score, kept))
    return ("red_panel_edges", pcx, pcy, pw, ph, corners, score)


def find_red_rings_target(img, paper, laser=None, panel=None):
    if not RED_RINGS_TARGET_ENABLED:
        return None
    if not paper and not panel:
        return None
    ...
    if (
        panel
        and not complete
        and RED_PANEL_CENTER_FALLBACK_ENABLED
        and red_panel_frame_is_complete(panel)
    ):
...
def red_panel_center_target(panel, red_target=None, require_red=True, require_complete=True):
    if (
        not RED_PANEL_RECT_CENTER_PRIORITY_ENABLED
        or not panel
    ):
        return None
    if require_complete and not red_panel_frame_is_complete(panel):
        return None
    if require_red and not red_target:
        return None
    _src, cx, cy, w, h, corners, score = panel
    red_score = 0
    out_src = "red_panel_center" if red_target else "red_panel_frame_center"
    if red_target:
        _r_src, _r_cx, _r_cy, _r_w, _r_h, _r_corners, red_score = red_target
    return (out_src, cx, cy, w, h, corners, int(score + red_score))
...
def find_square_target(img, laser, paper):
    ...
    panel_red = find_red_rings_target(img, paper, laser, panel) if panel else None
    if panel and panel_red:
        panel_target = red_panel_center_target(panel, panel_red, require_red=True)
        if panel_target:
            return panel_target
    if RED_RINGS_COMPLETE_AIM_ENABLED and panel_red and panel_red[0] == "red_rings":
        return panel_red

    global_red = find_red_rings_target(img, paper, laser, None) if not panel_red else None
    if global_red:
        if global_red[0] == "red_rings_part":
            if red_rings_part_control_allowed(global_red, laser):
                return global_red
            return None
        edge_panel = find_red_panel_edges_from_red_target(img, global_red, paper)
        if RED_RINGS_COMPLETE_AIM_ENABLED and global_red[0] == "red_rings" and edge_panel:
            return global_red
        panel_target = red_panel_center_target(
            edge_panel,
            global_red,
            require_red=True,
        ...
        if panel_target:
            return panel_target
        anchor_panel = find_anchor_panel_from_red_target(img, global_red, paper)
        panel_target = red_panel_center_target(
            anchor_panel,
            global_red,
            require_red=True,
        ...
        if panel_target:
            if RED_RINGS_COMPLETE_AIM_ENABLED and global_red[0] == "red_rings":
                return global_red
            return panel_target
        if RED_RINGS_DIRECT_AIM_FALLBACK_ENABLED and global_red[0] == "red_rings":
            _r_src, r_cx, r_cy, _r_w, _r_h, _r_corners, r_score = global_red
            edge_margin = min(r_cx, r_cy, FRAME_W - r_cx, FRAME_H - r_cy)
            if r_score >= RED_RINGS_DIRECT_AIM_MIN_SCORE and edge_margin >= RED_RINGS_DIRECT_AIM_MIN_EDGE_PX:
                return global_red

    panel_target = red_panel_center_target(panel, panel_red, require_red=False)
    if panel_target:
        return panel_target
...
def find_laser(img, paper=None, hint_x=None, hint_y=None, hint_max_dist=None):
    ...
    for b in blobs:
        ...
        if w > LASER_MAX_W or h > LASER_MAX_H:
            continue
        if paper and LASER_PAPER_RELAX_PX < 0:
...
def choose_laser_ref(img, paper, target=None, prev_lx=None, prev_ly=None):
    if FIXED_LASER_ENABLED:
        return fixed_laser_ref(target)

    real = None
    if REAL_LASER_PREFERRED:
        real = find_laser(img, paper, None, None, None)
        if real and real[6] >= REAL_LASER_MIN_SCORE:
            return real
...
def laser_point_on_target(target, lx, ly):
    return point_in_target_bounds(lx, ly, target, -LASER_ON_TARGET_RELAX_PX)


def laser_on_target(target, laser):
    if not target or not laser:
        return False
    _src, lx, ly, _w, _h, _pixels, _score = laser
    return laser_point_on_target(target, lx, ly)


def off_panel_recovery_needed(target, laser, lx=None, ly=None):
    if not OFF_PANEL_RECOVERY_ENABLED or not target or not laser:
        return False
    if target[0] not in ("red_panel_center", "red_panel_frame_center", "red_rings", "red_rings_part", "frame_center"):
        return False
    if lx is None or ly is None:
        _src, lx, ly, _w, _h, _pixels, _score = laser
    return not laser_point_on_target(target, lx, ly)


def fixed_laser_can_drive_target(target, laser):
    if not target or not laser:
        return False
    if laser[0] != "fixed":
        return True
    if target[0] == "red_rings_part":
        return red_rings_part_edge_command(target, laser) is not None
    return True
...
def shape_command_angle(yaw, pitch, err_x, err_y, target=None):
    ...
    if target and target[0] == "frame_center":
    ...
    elif target and (target[0] == "red_panel_center" or target[0] == "red_panel_frame_center"):
        ...
        if abs(err_x) <= RED_PANEL_CENTER_FINE_RADIUS_PX:
            yaw_max = RED_PANEL_CENTER_FINE_CONTROL_MAX_DEG
        if abs(err_y) <= RED_PANEL_CENTER_FINE_RADIUS_PX:
            pitch_max = RED_PANEL_CENTER_FINE_CONTROL_MAX_DEG
...
def compute_off_panel_recovery_command(target, laser, err_x, err_y):
    recovery_x = err_x
    recovery_y = err_y
    if abs(recovery_x) <= OFF_PANEL_RECOVERY_DEADBAND_PX:
        recovery_x = 0.0
    if abs(recovery_y) <= OFF_PANEL_RECOVERY_DEADBAND_PX:
        recovery_y = 0.0
    if abs(recovery_x) > abs(recovery_y) * OFF_PANEL_RECOVERY_MAJOR_AXIS_RATIO:
        recovery_y = 0.0
    elif abs(recovery_y) > abs(recovery_x) * OFF_PANEL_RECOVERY_MAJOR_AXIS_RATIO:
        recovery_x = 0.0

    raw_yaw, raw_pitch = pixel_delta_to_angle(recovery_x, recovery_y)
    img_yaw = shape_axis_command(
        raw_yaw,
        recovery_x,
        OFF_PANEL_RECOVERY_DEADBAND_PX,
        OFF_PANEL_RECOVERY_CONTROL_MAX_DEG,
    )
    img_pitch = shape_axis_command(
        raw_pitch,
        recovery_y,
        OFF_PANEL_RECOVERY_DEADBAND_PX,
        OFF_PANEL_RECOVERY_CONTROL_MAX_DEG,
    ...
    motor_yaw = img_pitch
    motor_pitch = -img_yaw
    return recovery_x, recovery_y, raw_yaw, raw_pitch, img_yaw, img_pitch, motor_yaw, motor_pitch
...
def limit_paper_center_command(target, yaw, pitch):
    if target and target[0] == "paper_center":
        ...
        yaw = -yaw
        pitch = -pitch
    elif target and (target[0] == "red_rings" or target[0] == "red_panel_center" or target[0] == "red_panel_frame_center"):
        yaw = -yaw
        pitch = -pitch
        if target[0] == "red_panel_center" or target[0] == "red_panel_frame_center":
...
def red_rings_part_edge_command(target, laser=None):
    if (
    ...
    if laser:
        _l_src, l_cx, l_cy, _l_w, _l_h, _l_pixels, _l_score = laser
        _t_src, t_cx, t_cy, _t_w, _t_h, _t_corners, _t_score = target
        err_x = l_cx - t_cx
        err_y = l_cy - t_cy
    ...
    ignore_err_gate = laser and laser[0] == "fixed"
    if bottom_clip and not top_clip and (ignore_err_gate or not laser or err_y < -RED_RINGS_PART_EDGE_ERR_PX):
        yaw = -RED_RINGS_PART_CONTROL_MAX_DEG
    elif top_clip and not bottom_clip and (ignore_err_gate or not laser or err_y > RED_RINGS_PART_EDGE_ERR_PX):
        yaw = RED_RINGS_PART_CONTROL_MAX_DEG

    if right_clip and not left_clip and (ignore_err_gate or not laser or err_x < -RED_RINGS_PART_EDGE_ERR_PX):
        pitch = RED_RINGS_PART_CONTROL_MAX_DEG
    elif left_clip and not right_clip and (ignore_err_gate or not laser or err_x > RED_RINGS_PART_EDGE_ERR_PX):
        pitch = -RED_RINGS_PART_CONTROL_MAX_DEG
...
def compute_motor_command(target, laser, err_x, err_y):
    if off_panel_recovery_needed(target, laser):
        return compute_off_panel_recovery_command(target, laser, err_x, err_y)

    edge_command = red_rings_part_edge_command(target, laser)
    if edge_command:
        raw_yaw, raw_pitch = pixel_delta_to_angle(err_x, err_y)
        motor_yaw, motor_pitch = edge_command
        return err_x, err_y, raw_yaw, raw_pitch, 0.0, 0.0, motor_yaw, motor_pitch

    if laser and laser[0] == "fixed" and FIXED_LASER_CLOSED_LOOP_ENABLED:
        raw_yaw, raw_pitch = pixel_delta_to_angle(err_x, err_y)
        img_yaw = shape_axis_command(
            raw_yaw,
            err_x,
...
def apply_pitch_guard(target, motor_yaw, motor_pitch):
    ...
    if _src == "red_rings" and cy <= PITCH_GUARD_SOFT_TARGET_CY and motor_yaw < 0.0:
        motor_yaw = 0.0
...
def target_ok_for_mode2(target, valid):
    if not target:
        return 0
    src, _cx, _cy, w, h, _corners, score = target
    if src == "red_panel_center":
        return 1
    if (
        RED_PANEL_MODE2_FRAME_FALLBACK_ENABLED
        and src == "red_panel_frame_center"
        and red_panel_frame_is_complete(target)
        and w >= RED_PANEL_MODE2_FRAME_MIN_W
        and h >= RED_PANEL_MODE2_FRAME_MIN_H
        and score >= RED_PANEL_MODE2_FRAME_MIN_SCORE
    ):
        return 1
    return 0


def send_packet(uart, valid, yaw, pitch, target_ok=0):
    # Extra target_ok keeps mode 2 search strict while older parsers can still
    # consume the first three fields.
    line = "$K230,%d,%.2f,%.2f,%d\n" % (valid, yaw, pitch, target_ok)
    uart.write(line)
...
def print_debug(
    ...
    if paper:
        ...
        paper_ok = 0
    print(
        "$DBG,valid=%d,mode=%s,paper=%d,px=%d,py=%d,pw=%d,ph=%d,p_score=%d,target=%s,laser=%s,on_target=%d,off_panel=%d,"
...
def diagnose_once():
    ...
    try:
        ...
        laser = choose_laser_ref(img, paper)
        target = None
        panel = None
        ...
        if target:
            laser = choose_laser_ref(img, paper, target)
            target = frame_part_recenter_target(target, laser)
            target = red_rings_part_recenter_target(target, laser)
        ...
        if target and laser:
            err_x = l_cx - t_cx
            err_y = l_cy - t_cy
            on_target = laser_on_target(target, laser)
            off_panel = off_panel_recovery_needed(target, laser)
            if laser[0] == "fixed" and not off_panel and not FIXED_LASER_CLOSED_LOOP_ENABLED:
                print(
                    "$DIAG_CMD,valid=0,laser_on_target=%d,off_panel=%d,reason=fixed_inside_hold,would_send=$K230,0,0.00,0.00"
                    % (1 if on_target else 0, 1 if off_panel else 0)
                )
            elif not fixed_laser_can_drive_target(target, laser):
                print(
                    "$DIAG_CMD,valid=0,laser_on_target=%d,off_panel=%d,reason=fixed_part_hold,would_send=$K230,0,0.00,0.00"
                    % (1 if on_target else 0, 1 if off_panel else 0)
                )
            else:
                ...
                print(
                    "$DIAG_CMD,valid=1,laser_on_target=%d,off_panel=%d,err_x=%.1f,err_y=%.1f,body_err_x=%.1f,body_err_y=%.1f,"
        ...
        while True:
            ...
            if FORCE_COMMAND_ENABLED:
                ...
                if rate_due(now, last_control_ms, CONTROL_SEND_HZ):
                    send_packet(uart, 1, FORCE_YAW, FORCE_PITCH, 0)
                    last_control_ms = now
            ...
            paper = find_paper(img, None)
            laser = choose_laser_ref(img, paper, None, prev_lx, prev_ly)
            if paper:
                target = find_square_target(img, laser, paper)
                target = frame_part_recenter_target(target, laser)
                target = red_rings_part_recenter_target(target, laser)
                ...
                target = find_square_target(img, laser, None)
            if target:
                laser = choose_laser_ref(img, paper, target, prev_lx, prev_ly)
                target = frame_part_recenter_target(target, laser)
                target = red_rings_part_recenter_target(target, laser)
            ...
            if target and laser:
                ...
                if (
                    ...
                    and _src0 != "red_panel_frame_center"
            ...
            if target and prev_tx is not None:
                ...
                if stable_src == "red_panel_center" or stable_src == "red_panel_frame_center":
                ...
                if laser and off_panel_recovery_needed(target, laser):
                    max_target_jump = OFF_PANEL_RECOVERY_MAX_TARGET_JUMP_PX
                if laser and laser[0] == "fixed" and (
                    _src == "red_panel_center" or _src == "red_panel_frame_center"
                ):
                    max_target_jump = OFF_PANEL_RECOVERY_MAX_TARGET_JUMP_PX
                jump_x = t_cx - prev_tx
                jump_y = t_cy - prev_ty
            ...
            if not target and laser and last_hold_target:
                now = time.ticks_ms()
                if OFF_PANEL_RECOVERY_TARGET_HOLD_ENABLED and time.ticks_diff(now, last_hold_ms) <= TARGET_HOLD_MS:
                    target = last_hold_target
                    using_held_target = True
            elif target and laser and last_hold_target and last_hold_target[0] == "red_rings":
                now = time.ticks_ms()
                if time.ticks_diff(now, last_hold_ms) <= TARGET_HOLD_MS and target[0] != "red_rings":
                    target = last_hold_target
                    using_held_target = True
            ...
            if target and laser:
                ...
                on_target = laser_point_on_target(target, filt_lx, filt_ly)
                off_panel = off_panel_recovery_needed(target, laser, filt_lx, filt_ly)
                body_err_x, body_err_y, raw_yaw, raw_pitch, yaw, pitch, motor_yaw, motor_pitch = compute_motor_command(
                    target,
                    laser,
                    err_x,
                ...
                if _t_src == "red_rings_part":
                    valid = 1
                if _t_src == "red_panel_center" or _t_src == "red_panel_frame_center":
                ...
                if off_panel:
                    valid = 1
                if not fixed_laser_can_drive_target(target, laser):
                    valid = 0
                if _l_src == "fixed" and not off_panel and not FIXED_LASER_CLOSED_LOOP_ENABLED:
                    valid = 0
                ...
                now = time.ticks_ms()
                mode2_target_ok = target_ok_for_mode2(target, valid)
                send_valid = CONTROL_OUTPUT_ENABLED and (valid or mode2_target_ok)
                ...
                if not target:
                    ...
                    if target_rejected and (
                        ...
                        or prev_target_src == "red_panel_frame_center"
                    ):
