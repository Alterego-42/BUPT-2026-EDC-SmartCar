...
SCRIPT_VERSION = "panel_center_v59_red_ring_yaw"
LOST_SEND_HZ = 15
CONTROL_SEND_HZ = 45
DEBUG_PRINT_HZ = 8
...
RED_RINGS_PART_CONTROL_ENABLED = False
...
FIXED_LASER_CX = 193.5
FIXED_LASER_CY = 54.0
FIXED_LASER_RANGE_REF_ENABLED = True
FIXED_LASER_FAR_TARGET_H = 150.0
FIXED_LASER_FAR_CX = 193.5
FIXED_LASER_FAR_CY = 54.0
FIXED_LASER_NEAR_TARGET_H = 220.0
FIXED_LASER_NEAR_CX = 193.3
FIXED_LASER_NEAR_CY = 62.4
...
CONFIG_PATH = "/sdcard/k230_target_config_v59.py"
...
def fixed_laser_ref(target=None):
    ...
    if FIXED_LASER_RANGE_REF_ENABLED and target:
        t_src, _t_cx, _t_cy, _t_w, t_h, _corners, _score = target
        ref_h = float(t_h)
        if t_src == "red_rings":
            ref_h *= FIXED_LASER_RED_RING_PANEL_H_SCALE
        span = FIXED_LASER_NEAR_TARGET_H - FIXED_LASER_FAR_TARGET_H
        if span != 0.0:
            ratio = clamp((ref_h - FIXED_LASER_FAR_TARGET_H) / span, 0.0, 1.0)
    ...
    return ("fixed", cx, cy, 4, 4, int(ref_h), int(ratio * 1000.0))
...
def red_panel_center_target(panel, red_target=None, require_red=True, require_complete=True):
    ...
    return ("red_panel_center", cx, cy, w, h, corners, int(score + red_score))
...
def diagnose_once():
    ...
    try:
        ...
        if FIXED_LASER_ENABLED and target:
            laser = fixed_laser_ref(target)
            target = frame_part_recenter_target(target, laser)
            target = red_rings_part_recenter_target(target, laser)
